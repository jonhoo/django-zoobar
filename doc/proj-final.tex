...
if user.person.zoobars > 5:
  ...
  To address the first obstacle, we map database object's properties
  to new concolic values, e.g., creating a new concolic int for
  \textit{user.person.zoobars} when \textit{user} is assigned. To
  explore more program branches, we automatically create new database
  entries for concrete values that are absent, e.g., when Z3 generates
  a new value for \textit{user.person.zoobars}, say 7, we ensure that
  there is a database entry with that value.

  Compared to \underline{All}, we achieve slightly greater coverage
  and in slightly fewer iterations, which is promising. The downside
  is that dynamically adding values to the database makes it more
  difficult to write program invariants.
\end{enumerate}
